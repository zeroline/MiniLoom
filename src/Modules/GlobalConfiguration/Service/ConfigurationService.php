...
namespace zeroline\MiniLoom\Modules\GlobalConfiguration\Service;
...
use zeroline\MiniLoom\Data\Database\SQL\ValidationModel;
        ...
        if (!is_null($sector->getSchema()) && !empty($sector->getSchema())) {
            ...
            if (array_key_exists($section->getIdentifier(), $schemaArray)) {
                ...
                if (array_key_exists($identifier, $schemaSubArray)) {
                    ...
                    $validatedModel = new ValidationModel(array(
                        $identifier => $content
                    ));
